...
export default function ProductoDetalle({ params }: PageProps) {
  ...
  // Zustand Stores
  const { currentProduct: product, isLoading, fetchProductById } = useProductStore();
  ...
  // Actualizar imagen principal cuando el producto se cargue
  useEffect(() => {
    if (product?.image) {
  ...
  if (isLoading) return (
    <div className="h-screen flex items-center justify-center transition-colors duration-300"
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <p className="text-[10px] font-black tracking-[0.3em] animate-pulse uppercase">Sincronizando con Servidor...</p>
      </div>
    </div>
  );
