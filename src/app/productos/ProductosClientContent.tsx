...
export default function ProductosClientContent({ initialProducts = [], activeCategory }: Props) {
  ...
  // 1. Efecto de Hidratación
  useEffect(() => {
    setMounted(true);
    // Simular carga inicial del catálogo
    const timer = setTimeout(() => setIsPageLoading(false), 800);
    return () => clearTimeout(timer);
  }, []);

  // 2. EFECTO PARA RESETEAR SCROLL Y CARGAR PRODUCTOS
  useEffect(() => {
    window.scrollTo(0, 0);
  ...
  // FUNCIÓN CORREGIDA CON LOADER
  const volverAlCatalogo = () => {
    // 1. Mostramos el loader inmediatamente como "almohada"
    setIsPageLoading(true); 
    setSearchQuery("");
  
    // 2. Navegamos sin esperas artificiales largas
    // El loader se irá solo cuando el nuevo componente se monte
    router.push('/productos');
    
    // Opcional: un failsafe por si la navegación es instantánea
  ...
  return (
    <>
      ...
      <div className={`min-h-screen max-w-7xl mx-auto px-4 pt-24 pb-20 transition-all duration-700 ${
        isPageLoading ? 'opacity-0 scale-95 blur-xl' : 'opacity-100 scale-100 blur-0'
      }`}>
