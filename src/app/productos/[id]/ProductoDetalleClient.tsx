...
export default function ProductoDetalleClient({ product }: ProductoDetalleClientProps) {
  ...
  // Nuevo estado para controlar la salida suave
  ...
  // --- FUNCIÓN DE NAVEGACIÓN CON SALIDA SUAVE ---
  const volverAlCatalogo = () => {
    setIsExiting(true); // Iniciamos el desvanecimiento
    setTimeout(() => {
      router.push('/productos');
    }, 150); // Tiempo mínimo para que el ojo perciba la salida pero no sea lento
  };

  if (!mounted) return null;
  if (!product) return <div className="pt-40 text-center font-black uppercase opacity-20">Producto no encontrado</div>;
  ...
  return (
    <motion.div 
      ...
      animate={{ 
        opacity: isExiting ? 0 : 1, // Si está saliendo, se desvanece
        y: isExiting ? -10 : 0       // Si está saliendo, sube un poco
      }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start">
        <div className="lg:col-span-7 space-y-6">
          <div
            ...
            className={`relative aspect-square rounded-[2.5rem] sm:rounded-[3rem] overflow-hidden bg-neutral-100 dark:bg-neutral-900 border border-[var(--border-theme)] shadow-2xl group transition-all duration-300 ${isUnlockedForever ? 'cursor-crosshair' : 'cursor-zoom-in'}`}
          >
            <Image
              src={mainImage}
              alt={product.name}
              ...
              className={`object-contain p-8 sm:p-12 transition-all duration-700 ${isImageLoading ? 'scale-95 blur-md opacity-0' : 'scale-100 blur-0 opacity-100'} ${isHovering && isUnlockedForever ? 'opacity-0' : 'opacity-100'}`}
            />
            ...
            {esOferta && (
              <div className="absolute top-8 left-8 z-20 bg-blue-600 text-white px-4 py-1.5 rounded-full font-black text-[9px] uppercase tracking-widest flex items-center gap-2 shadow-lg">
          ...
          <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
            {allImages.map((img, i) => (
              <button
                ...
                className={`relative w-20 h-24 sm:w-24 sm:h-28 rounded-[2rem] overflow-hidden bg-white dark:bg-neutral-800 border transition-all duration-500 p-3 shrink-0 flex items-center justify-center ${mainImage === img ? 'border-blue-600 ring-4 ring-blue-600/15' : 'opacity-40 hover:opacity-100 border-[var(--border-theme)]'}`}
              >
                <div className="relative w-full h-full">
                  <Image src={img} alt={`thumb-${i}`} fill quality={60} className="object-contain" />
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="lg:col-span-5 flex flex-col gap-8 lg:sticky lg:top-32 pt-2">
          <header className="space-y-4">
            <div className="flex items-center gap-3 opacity-60">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[var(--foreground)]">{product.category || "Hardware"}</p>
            </div>
          ...
          <p className="text-[var(--foreground)] opacity-60 leading-relaxed text-base italic">
            {product.description}
          </p>
          ...
          <div className="space-y-4 pt-4">
            <button
              ...
              disabled={!hasStock}
              className={`w-full h-20 rounded-full font-black text-[11px] uppercase tracking-[0.4em] transition-all border-2 shadow-2xl hover:scale-[1.02] active:scale-[0.98] ${hasStock ? 'bg-blue-600 text-white border-blue-600 hover:bg-blue-700 shadow-blue-600/30' : 'bg-neutral-800 text-neutral-500 border-neutral-800 cursor-not-allowed opacity-50'}`}
            >
            ...
            <button
              ...
              disabled={!hasStock}
              className="w-full h-20 rounded-full font-black text-[11px] uppercase tracking-[0.4em] transition-all border-2 border-[var(--border-theme)]/80 hover:bg-[var(--foreground)] hover:text-[var(--background)]"
            >
