  ...
  return (
    <div className="p-4 rounded-xl shadow-sm flex items-center gap-4 border transition-all duration-300"
      ...
      <div className="flex-grow">
        <h3 className="font-medium text-sm md:text-base" style={{ color: 'var(--foreground)' }}>{item.name}</h3>
        <div className="flex items-center justify-between mt-4">
          ...
          <button onClick={() => removeFromCart(item.id, userId)} 
                  className="opacity-40 hover:opacity-100 hover:text-red-500 transition-all"><Trash2 size={20} /></button>
        </div>
      </div>
...
export default function CarritoPage() {
  ...
  useEffect(() => {
    if (isMounted && products.length > 0) revalidateCartStock(products);
  ...
  if (cart.length === 0) {
    return (
      <div className="min-h-screen pt-32 flex flex-col items-center" style={{ backgroundColor: 'var(--background)' }}>
        <div className="p-10 rounded-3xl border text-center max-w-md w-full mx-4 shadow-xl" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--border-theme)' }}>
          ...
          <Link href="/" className="bg-blue-600 text-white px-8 py-3 rounded-xl font-bold block mt-6 hover:bg-blue-500 transition-all">Ir a la tienda</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen pt-28 pb-12 px-4" style={{ backgroundColor: 'var(--background)' }}>
      <div className="max-w-6xl mx-auto">
        <h1 className="text-2xl font-bold mb-8" style={{ color: 'var(--foreground)' }}>Carrito de compras</h1>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-4">
            {cart.map((item) => (
              <CartItemRow key={item.id} item={item} updateQuantity={updateQuantity} removeFromCart={removeFromCart} userId={user?.id} />
            ))}
          </div>
          <div className="lg:col-span-1">
            <div className="p-6 rounded-2xl border sticky top-28 shadow-xl" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--border-theme)' }}>
              ...
              <button onClick={() => { setIsLoading(true); setTimeout(() => router.push("/checkout"), 1000); }} 
                      disabled={isLoading}
                      className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold mt-6 hover:bg-blue-500 disabled:opacity-50 flex items-center justify-center">
