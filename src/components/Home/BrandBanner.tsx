...
export const BrandBanner = () => {
  ...
  // Evitamos que el servidor y el cliente rendericen cosas distintas
  useEffect(() => {
    setIsMounted(true);
  }, []);

  if (!isMounted) return null; // O un esqueleto/placeholder
    ...
    <section className="py-16 border-b overflow-hidden relative border-[var(--border-theme)]">
      <div className="max-w-7xl mx-auto px-6 mb-12">
        <div className="flex items-center gap-4 opacity-40">
          ...
          <p className="text-[10px] font-black tracking-[0.4em] uppercase whitespace-nowrap">
      ...
      <div className="relative flex items-center">
        {/* Usamos tus gradientes con variables CSS */}
        ...
        {/* Aplicamos las clases definidas en el CSS v4 */}
        <div className="flex animate-marquee pause-marquee whitespace-nowrap gap-20 items-center">
            ...
            <div key={`${brand.name}-${index}`} className="group flex flex-col items-center justify-center min-w-[140px]">
              <div className="h-12 flex items-center justify-center">
                <img 
                  src={brand.logo} 
                  alt={brand.name} 
                  className="h-full w-auto object-contain grayscale invert-0 dark:invert-[0.8] opacity-50 group-hover:grayscale-0 group-hover:invert-0 group-hover:opacity-100 transition-all duration-500 scale-90 group-hover:scale-110"
                />
              </div>
